...
{
public:
  BaseVariantImpl (void *ptr, wsql::SQLString vtype) :
    cvptr(ptr),
    vTypeName(vtype)
  {}
  ...
  T* get() const {
    ...
    if ((vTypeName == typeid(std::string).name() &&
      typeid(T).name() == typeid(wsql::SQLString).name()) ||
      (vTypeName == typeid(wsql::SQLString).name() &&
      typeid(T).name() == typeid(std::string).name())    ||
      (vTypeName == typeid(std::map< std::string, std::string >).name() &&
      typeid(T).name() ==
        typeid(std::map< wsql::SQLString, wsql::SQLString >).name()) ||
      (vTypeName ==
        typeid(std::map< wsql::SQLString, wsql::SQLString >).name() &&
      typeid(T).name() ==
      ...
      typeid(T).name() ==
        typeid(std::list< wsql::SQLString >).name()) ||
      (vTypeName ==
        typeid(std::list< wsql::SQLString >).name() &&
      typeid(T).name() ==
    ...
    if (typeid(T).name() != vTypeName) {
      throw wsql::InvalidArgumentException("Variant type doesn't match.");
    }
    ...
    if (tmp) {
      ...
      while(cit != tmp->end()) {
        (static_cast< T * >(cvptr))->insert(
            std::make_pair(wsql::SQLString(cit->first),
                    wsql::SQLString(cit->second)));
        ++cit;
      }
    }
  }
...
private:
  ...
  void copy_content(VariantList& var)
  {
    ...
    if (tmp) {
      ...
      typename T::const_iterator cit=tmp->begin();
      while(cit != tmp->end()) {
        (static_cast< T * >(cvptr))->push_back(wsql::SQLString(*cit));
        ++cit;
      }
    }
  }
...
public:
  ...
  Variant(const char* i) :
    variant(new VariantImpl< wsql::SQLString >(i)) {}

  Variant(const std::string &i) :
    variant(new VariantImpl< wsql::SQLString >(i)) {}

  Variant(const wsql::SQLString &i) :
    variant(new VariantImpl< wsql::SQLString >(i)) {}

  Variant(const std::list< std::string > &i) :
    variant(new VariantList< std::list < std::string > >(i)) {}

  Variant(const std::list< wsql::SQLString > &i) :
    variant(new VariantList< std::list < wsql::SQLString > >(i)) {}

  Variant(const std::map< std::string, std::string > &i) :
    variant(new VariantMap< std::map< std::string, std::string > >(i)) {}

  Variant(const std::map< wsql::SQLString, wsql::SQLString > &i) :
    variant(new VariantMap< std::map< wsql::SQLString, wsql::SQLString > >(i)) {}
