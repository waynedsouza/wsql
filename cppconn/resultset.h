...
public:
	...
	virtual uint32_t findColumn(const wsql::SQLString& columnLabel) const = 0;

	virtual bool first() = 0;

	virtual std::istream * getBlob(uint32_t columnIndex)  const = 0;
	virtual std::istream * getBlob(const wsql::SQLString& columnLabel) const = 0;

	virtual bool getBoolean(uint32_t columnIndex) const = 0;
	virtual bool getBoolean(const wsql::SQLString& columnLabel) const = 0;

	virtual int getConcurrency() = 0;
	virtual SQLString getCursorName() = 0;

	virtual long double getDouble(uint32_t columnIndex) const = 0;
	virtual long double getDouble(const wsql::SQLString& columnLabel) const = 0;
	...
	virtual int32_t getInt(const wsql::SQLString& columnLabel) const = 0;

	virtual uint32_t getUInt(uint32_t columnIndex) const = 0;
	virtual uint32_t getUInt(const wsql::SQLString& columnLabel) const = 0;

	virtual int64_t getInt64(uint32_t columnIndex) const = 0;
	virtual int64_t getInt64(const wsql::SQLString& columnLabel) const = 0;

	virtual uint64_t getUInt64(uint32_t columnIndex) const = 0;
	virtual uint64_t getUInt64(const wsql::SQLString& columnLabel) const = 0;
	...
	virtual RowID * getRowId(const wsql::SQLString & columnLabel) = 0;

	virtual const Statement * getStatement() const = 0;

	virtual SQLString getString(uint32_t columnIndex)  const = 0;
	virtual SQLString getString(const wsql::SQLString& columnLabel) const = 0;
	...
	virtual bool isNull(const wsql::SQLString& columnLabel) const = 0;
